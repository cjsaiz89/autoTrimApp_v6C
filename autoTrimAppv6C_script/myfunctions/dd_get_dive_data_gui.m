...
function [loginfo,eng,results] = dd_get_dive_data(id_dive,require_results,dataDir)
...
% assumes basename looks like pGGGYYYY and we are cd'd to the proper dir
    ...
    % KUTI
%     if 1
%         % all callers should specify require_results = 1
%         % (get sg_cal constants from nc file) 
%         sgc_exists = 0;
%         require_results = 1; % require loading from nc file
%         if (nargin >= 3) % CJS nargin is num. of input arguments in function dd_get_dive_data 
%             basename = sprintf('p%s',id_dive);
%             basename = fullfile(dataDir, basename);
% %             disp(basename); % CJS
%             %basename = sprintf('%s\%s', dataDir, baseName);
%         else
%             basename = sprintf('p%s',id_dive);
%         end
    
%  CJS commented - else condition is never reached        
%     else
%         if (nargin < 2)
%           require_results = 1; % require loading from nc file
%         end
...
%     end
    % CJS replaces the first part of conditionals

%     nc_filename = sprintf('%s.nc',basename); % CJS this should be the complete path finally used ...\pGGGDDDD.nc
    
    nc_filename = fullfile(dataDir,strcat('p',id_dive,'.nc'));
    basename = erase(nc_filename,'.nc');
    nc_fileinfo = dir(nc_filename); % CJS get file metadata
    ...
    if (nc_exists)
      try
        ...
        loginfo.version = globals.seaglider_software_version;
