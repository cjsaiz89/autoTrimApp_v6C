...
optode_present = false;
if (isfield(results,'aanderaa3830_qc'))
    optode_present = true;
...
elseif( isfield(results,'aanderaa4330_qc'))
    optode_present = true;
...
elseif(isfield(results,'aanderaa4831_qc'))
    optode_present = true;
    optode_qc = str2num(results.aanderaa4831_qc);
    optode_time = results.aanderaa4831_results_time - results.aanderaa4831_results_time(1); % elapsed
    optode_instrument_oxygen = results.aanderaa4831_instrument_dissolved_oxygen;
    optode_oxygen_qc = results.aanderaa4831_dissolved_oxygen_qc;
    optode_oxygen = results.aanderaa4831_dissolved_oxygen;
end

% Contros HydroFlash O2 sensor 
contros_optode_present = isfield(results,'contopt_dissolved_oxygen');
if (contros_optode_present)
  contros_optode_time = results.contopt_results_time - results.contopt_results_time(1); % elapsed
  contros_optode_oxygen_qc = results.contopt_dissolved_oxygen_qc;
  contros_optode_oxygen = results.contopt_dissolved_oxygen; % computed with glider temp & salinity
  contros_optode_instrument_oxygen = results.contopt_instrument_dissolved_oxygen; % computed with sensor temp & glider salinity
end

%Rinko ARO-FT sensor
aroft_optode_present = isfield(results, 'aroft_time' ); 
if ( aroft_optode_present )
  %aroft_optode_time = results.aroft_results_time - results.aroft_results_time(1); % elapsed
  aroft_optode_time = results.aroft_time - results.aroft_time(1); % elapsed
  aroft_optode_oxygen_qc = results.aroft_dissolved_oxygen_qc;
  aroft_optode_oxygen = results.aroft_dissolved_oxygen;
  %aroft_optode_instrument_oxygen = results.aroft_instrument_dissolved_oxygen; % computed with glider temp & salinity
end
...
%
% 30-Sep-2016 jfaust - support wetlabs sensors attached to scicon.
%

wetlabs_sensors.bb2fl_present = 0;
wetlabs_sensors.bbfl2_present = 0;
wetlabs_sensors.fl_present = 0;
wetlabs_sensors.fl3_present = 0;
wetlabs_sensors.bb3_present = 0;
wetlabs_sensors.flntu_present = 0;
wetlabs_sensors.ntufl2_present = 0;
wetlabs_sensors.wlseaowl_present = 0;

wetlabs_sensors.bb2fl_present_sc = 0;               % for wetlabs attached to scicon
wetlabs_sensors.bbfl2_present_sc = 0;
wetlabs_sensors.fl_present_sc = 0;
wetlabs_sensors.fl3_present_sc = 0;
wetlabs_sensors.bb3_present_sc = 0;
wetlabs_sensors.flntu_present_sc = 0;
wetlabs_sensors.ntufl2_present_sc = 0;
wetlabs_sensors.wlseaowl_present_sc = 0;

% 14-Mar-2018 - jfaust - Satlantic PAR
par_satd_present = 0;
par_satd_present_sc = 0;


if (isfield(eng,'wlbb2fl_BB1sig') || ...
        isfield(eng,'wlbb2flvmt_BB1sig'))           % old - used on SG558 refurb
    wetlabs_sensors.bb2fl_present = 1;
end
if (isfield(results,'wlbb2fl_BB1sig'))
    wetlabs_sensors.bb2fl_present_sc = 1;
end

if (isfield(eng,'wlbbfl2_BB1sig') || ...
        isfield(eng,'wlbbfl2_BBsig') || ...         % old
        isfield(eng,'wlbbfl2vmt_BBsig'))            % old - used on SG602-SG606
    wetlabs_sensors.bbfl2_present = 1;
end
if isfield(results,'wlbbfl2_BB1sig')
    wetlabs_sensors.bbfl2_present_sc = 1;
end

if isfield(eng,'wlfl_FL1sig')
    wetlabs_sensors.fl_present = 1;
end
if isfield(results, 'wlfl_FL1sig')
    wetlabs_sensors.fl_present_sc = 1;
end

if isfield(eng,'wlfl3_FL1sig')
    wetlabs_sensors.fl3_present = 1;
end
if isfield(results, 'wlfl3_FL1sig')
    wetlabs_sensors.fl3_present_sc = 1;
end

if isfield(eng,'wlbb3_BB1sig')
    wetlabs_sensors.bb3_present = 1;
end
if isfield(results,'wlbb3_BB1sig')
    wetlabs_sensors.bb3_present_sc = 1;
end
...
if isfield(eng, 'wlflntu_FL1sig')
    wetlabs_sensors.flntu_present = 1;
end
if isfield(results,'wlflntu_FL1sig')
    wetlabs_sensors.flntu_present_sc = 1;
end

if isfield(eng, 'wlntufl2_FL1sig')
    wetlabs_sensors.ntufl2_present = 1;
end
if isfield(results,'wlntufl2_FL1sig')
    wetlabs_sensors.ntufl2_present_sc = 1;
end

if isfield(eng, 'wlseaowl_FL1sig')
    wetlabs_sensors.wlseaowl_present = 1;
end
if isfield(results,'wlseaowl_FL1sig')
    wetlabs_sensors.wlseaowl_present_sc = 1;
end

% PAR - satlantic
if (isfield(eng, 'satd_PARuV'))
    par_satd_present = 1;
end
if (isfield(results, 'satd_PARuV'))
    par_satd_present_sc = 1;
end
